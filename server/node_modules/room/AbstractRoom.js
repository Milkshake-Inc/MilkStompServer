...
function AbstractRoom(id, type, capacity) {
	...
	this.heartbeat = setInterval(this.sendHeartbeat.bind(this), heartBeatInterval);
}
...
Object.defineProperty(AbstractRoom.prototype, 'players', {
	get: function () {
		return _.toArray(this.playerStore);
	}
});

AbstractRoom.prototype.sendTo = function (players, packet) {
	for (var i = 0; i < players.length; i++) {
		if (this.getPlayerById(players[i].id) !== null) players[i].send(packet);
	}
};

AbstractRoom.prototype.sendToAll = function (packet, excludedUser) {
	var players = excludedUser ? _(this.players).remove(function (e) {
		return e.id == excludedUser.id;
	}) : this.players;

	this.sendTo(players, packet);
};
...
AbstractRoom.prototype.addPlayer = function addUser(player) {
	if (_.has(this.playerStore, player.id)) {
		...
		player.room = this;
		this.sendToAll(Packet.playerCreatePacket(player));
...
AbstractRoom.prototype.removePlayer = function removePlayer(player) {
	if (_.has(this.playerStore, player.id)) {
		...
		player.room = null;
		this.sendToAll(Packet.playerRemovePacket(player));
	} else {
...
AbstractRoom.prototype.sendHeartbeat = function () {
	this.sendToAll(Packet.heartbeat());
};
